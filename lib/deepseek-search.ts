...
export async function searchDeepSeek(query: string): Promise<FoodSearchResult | null> {
  ...
  const prompt = `Для продукта или блюда «${query.trim()}» укажи БЖУ на 100 грамм. Ответь только одним валидным JSON-объектом без markdown и без пояснений, в формате: {"name": "название продукта", "carbsPer100g": число, "proteinPer100g": число, "fatPer100g": число}. Все числа — только цифры (можно с точкой), без единиц измерения. Если не уверен — дай разумную оценку.`;
