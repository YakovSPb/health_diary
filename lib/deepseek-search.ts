...
/** Распознавание еды по фото (требуется модель с vision, например gpt-4o-mini при DEEPSEEK_API_URL на openai.com). */
export async function parseFoodFromImage(
  imageDataUrl: string
): Promise<ParsedFood> {
  const apiKey = process.env.DEEPSEEK_API_KEY?.trim();
  const { url, model, baseUrl } = getDeepSeekConfig();

  if (!apiKey) {
    throw new Error(
      'DEEPSEEK_API_KEY не задан. Для распознавания по фото добавьте ключ и укажите DEEPSEEK_API_URL на API с поддержкой vision (например OpenAI).'
    );
  }

  if (!baseUrl.includes('openai.com')) {
    throw new Error(
      'Распознавание по фото поддерживается только при DEEPSEEK_API_URL, указывающем на OpenAI API (модель с vision, например gpt-4o-mini).'
    );
  }
